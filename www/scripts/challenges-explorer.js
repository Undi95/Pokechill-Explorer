...
addChallenge({
    ...
    check: (data) => {
        ...
        let count = 0;
        for (let key in data) {
            // Les vraies méga-évolutions: 'mega' suivi d'une majuscule (megaCharizard, megaRaichuX...)
            if (/^mega[A-Z]/.test(key) && data[key]?.caught > 0) count++;
        }
        return count;
    }
});
...
addChallenge({
    ...
    check: (data) => {
        ...
        let count = 0;
        for (let key in data) {
            // Les vraies méga-évolutions shiny: 'mega' suivi d'une majuscule
            if (/^mega[A-Z]/.test(key) && data[key]?.caught > 0 && data[key]?.shiny) count++;
        }
        return count;
    }
});
...
addChallenge({
    id: 'first_zcrystal',
    title: { fr: 'Premier Cristal Z', en: 'First Z-Crystal' },
    description: { fr: 'Cristaux Z collectés', en: 'Z-Crystals collected' },
    icon: 'normaliumZ',
    category: CHALLENGE_CATEGORIES.ITEMS,
    tiers: [1, 3, 6, 9, 12, 18],
    isNumeric: true,
    check: (data) => countZCrystals(data)
});

addChallenge({
    id: 'zcrystals_x20',
    title: { fr: 'Collection de Cristaux Z', en: 'Z-Crystal Collection' },
    description: { fr: 'Cristaux Z différents à x20', en: 'Different Z-Crystals at x20' },
    icon: 'firiumZ',
    category: CHALLENGE_CATEGORIES.ITEMS,
    tiers: [3, 6, 9, 12, 15, 18],
    isNumeric: true,
    check: (data) => countZCrystalsAt20(data)
});
...
addChallenge({
    ...
    category: CHALLENGE_CATEGORIES.SECRET,
    tiers: [1],
    check: (data) => (data?.secretAerodactyl?.caught > 0 || data?.aerodactylB?.caught > 0 || data?.betaAerodactyl?.caught > 0) ? 1 : 0
});
...
addChallenge({
    ...
    category: CHALLENGE_CATEGORIES.SECRET,
    tiers: [1],
    check: (data) => ((data?.secretAerodactyl?.caught > 0 && data?.secretAerodactyl?.shiny) || (data?.aerodactylB?.caught > 0 && data?.aerodactylB?.shiny)) ? 1 : 0
});
...
addChallenge({
    id: 'starsign_first',
    title: { fr: 'Premier Star Sign', en: 'First Star Sign' },
    description: { fr: 'Pokémon avec au moins un Star Sign', en: 'Pokémon with at least one Star Sign' },
    icon: 'sol',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 5, 10, 25, 50, 100],
    isNumeric: true,
    check: (data) => countPokemonWithStarSign(data)
});

addChallenge({
    id: 'starsign_complete',
    title: { fr: 'Collection Complète', en: 'Complete Collection' },
    description: { fr: 'Pokémon avec tous les Star Signs (6/6)', en: 'Pokémon with all Star Signs (6/6)' },
    icon: 'shinyCharm',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 3, 5, 10, 20, 50],
    isNumeric: true,
    priorityTop: true,
    check: (data) => countPokemonWithCompleteStarSign(data)
});

addChallenge({
    id: 'starsign_sol',
    title: { fr: 'Star Sign: Sol', en: 'Star Sign: Sol' },
    description: { fr: 'Pokémon possédant le Star Sign Sol', en: 'Pokémon with Sol Star Sign' },
    icon: 'sol',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 3, 5, 10, 20, 50],
    isNumeric: true,
    check: (data) => countPokemonWithSpecificStarSign(data, 'sol')
});

addChallenge({
    id: 'starsign_luna',
    title: { fr: 'Star Sign: Luna', en: 'Star Sign: Luna' },
    description: { fr: 'Pokémon possédant le Star Sign Luna', en: 'Pokémon with Luna Star Sign' },
    icon: 'luna',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 3, 5, 10, 20, 50],
    isNumeric: true,
    check: (data) => countPokemonWithSpecificStarSign(data, 'luna')
});

addChallenge({
    id: 'starsign_pluto',
    title: { fr: 'Star Sign: Pluto', en: 'Star Sign: Pluto' },
    description: { fr: 'Pokémon possédant le Star Sign Pluto', en: 'Pokémon with Pluto Star Sign' },
    icon: 'pluto',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 3, 5, 10, 20, 50],
    isNumeric: true,
    check: (data) => countPokemonWithSpecificStarSign(data, 'pluto')
});

addChallenge({
    id: 'starsign_ceres',
    title: { fr: 'Star Sign: Ceres', en: 'Star Sign: Ceres' },
    description: { fr: 'Pokémon possédant le Star Sign Ceres', en: 'Pokémon with Ceres Star Sign' },
    icon: 'ceres',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 3, 5, 10, 20, 50],
    isNumeric: true,
    check: (data) => countPokemonWithSpecificStarSign(data, 'ceres')
});

addChallenge({
    id: 'starsign_terra',
    title: { fr: 'Star Sign: Terra', en: 'Star Sign: Terra' },
    description: { fr: 'Pokémon possédant le Star Sign Terra', en: 'Pokémon with Terra Star Sign' },
    icon: 'terra',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 3, 5, 10, 20, 50],
    isNumeric: true,
    check: (data) => countPokemonWithSpecificStarSign(data, 'terra')
});

addChallenge({
    id: 'starsign_eris',
    title: { fr: 'Star Sign: Eris', en: 'Star Sign: Eris' },
    description: { fr: 'Pokémon possédant le Star Sign Eris', en: 'Pokémon with Eris Star Sign' },
    icon: 'eris',
    category: CHALLENGE_CATEGORIES.STARSIGN,
    tiers: [1, 3, 5, 10, 20, 50],
    isNumeric: true,
    check: (data) => countPokemonWithSpecificStarSign(data, 'eris')
});
...
// === NOUVEAUX CHALLENGES V2.1.1 ===

// Evolution Master - Familles complètes
addChallenge({
    id: 'evolution_master',
    title: { fr: 'Maître de l\'Évolution', en: 'Evolution Master' },
    description: { fr: 'Familles d\'évolution complètes (pré-évo + évolué + final)', en: 'Complete evolution families (pre-evo + mid + final)' },
    icon: 'rareCandy',
    category: CHALLENGE_CATEGORIES.POKEMON,
    tiers: [10, 25, 50, 75, 100, 150],
    isNumeric: true,
    check: (data) => countCompleteEvolutionFamilies(data)
});

// Shiny Legendary
addChallenge({
    id: 'shiny_legendary',
    title: { fr: 'Légendaires Shiny', en: 'Shiny Legendaries' },
    description: { fr: 'Légendaires capturés en version Shiny', en: 'Legendary Pokémon caught in Shiny version' },
    icon: 'shinyCharm',
    category: CHALLENGE_CATEGORIES.SHINY,
    tiers: [1, 3, 6, 10, 15, 25],
    isNumeric: true,
    check: (data) => countShinyLegendaries(data)
});

// Starter Master - All starter Pokémon
addChallenge({
    id: 'starter_master',
    title: { fr: 'Maître des Starters', en: 'Starter Master' },
    description: { fr: 'Tous les starters de base capturés (27)', en: 'All base starter Pokémon caught (27)' },
    icon: 'pokeball',
    category: CHALLENGE_CATEGORIES.POKEMON,
    tiers: [5, 10, 15, 20, 23, 27],
    isNumeric: true,
    check: (data) => countStarters(data)
});

// Premium Shop Items (Noigrume Shop)
addChallenge({
    id: 'premium_shop_items',
    title: { fr: 'Items Premium (Noigrume)', en: 'Premium Items (Apricorn)' },
    description: { fr: 'Items premium du shop Noigrume collectés', en: 'Premium Apricorn shop items collected' },
    icon: 'replicatorUpgradeS',
    category: CHALLENGE_CATEGORIES.ITEMS,
    tiers: [1, 2, 3, 4, 5],
    isNumeric: true,
    check: (data) => countPremiumShopItems(data)
});

// Starter Evolution Master - All starters and their evolutions
addChallenge({
    id: 'starter_evolution_master',
    title: { fr: 'Maître Évolution Starters', en: 'Starter Evolution Master' },
    description: { fr: 'Toutes les évolutions de starters (81)', en: 'All starter evolutions caught (81)' },
    icon: 'masterBall',
    category: CHALLENGE_CATEGORIES.POKEMON,
    tiers: [20, 40, 60, 81],
    isNumeric: true,
    check: (data) => countStarterEvolutions(data)
});

// Level 100 Club
addChallenge({
    id: 'level_100_club',
    title: { fr: 'Club Niveau 100', en: 'Level 100 Club' },
    description: { fr: 'Pokémon atteignant le niveau 100', en: 'Pokémon reaching level 100' },
    icon: 'machoBrace',
    category: CHALLENGE_CATEGORIES.MISC,
    tiers: [5, 15, 30, 60, 100, 200],
    isNumeric: true,
    check: (data) => countLevel100(data)
});
...
addChallenge({
    id: 'eeveelutions',
    title: { fr: 'Collection Evoli', en: 'Eevee Collection' },
    description: { fr: 'Toutes les évolutions d\'Evoli capturées', en: 'All Eevee evolutions caught' },
    icon: 'eevee',
    category: CHALLENGE_CATEGORIES.POKEMON,
    tiers: [3, 5, 7, 9],
    isNumeric: true,
    check: (data) => countEeveelutions(data)
});

// Fossil Collector
addChallenge({
    id: 'fossil_collector',
    title: { fr: 'Collectionneur de Fossiles', en: 'Fossil Collector' },
    description: { fr: 'Pokémon fossiles ressuscités', en: 'Resurrected fossil Pokémon' },
    icon: 'helixFossil',
    category: CHALLENGE_CATEGORIES.POKEMON,
    tiers: [3, 6, 10, 15, 20, 28],
    isNumeric: true,
    check: (data) => countFossilPokemon(data)
});

// Pseudo-Legendaries
addChallenge({
    id: 'pseudo_legendaries',
    title: { fr: 'Pseudo-Légendaires', en: 'Pseudo-Legendaries' },
    description: { fr: 'Pseudo-légendaires capturés (BST 600)', en: 'Pseudo-legendary Pokémon caught (BST 600)' },
    icon: 'dragonite',
    category: CHALLENGE_CATEGORIES.POKEMON,
    tiers: [3, 6, 9, 12, 15, 20],
    isNumeric: true,
    check: (data) => countPseudoLegendaries(data)
});

// Baby Pokemon
addChallenge({
    id: 'baby_collector',
    title: { fr: 'Collection Bébé', en: 'Baby Collection' },
    description: { fr: 'Pokémon bébé capturés', en: 'Baby Pokémon caught' },
    icon: 'pichu',
    category: CHALLENGE_CATEGORIES.POKEMON,
    tiers: [5, 10, 15, 20, 25, 35],
    isNumeric: true,
    check: (data) => countBabyPokemon(data)
});
...
function countPokemonWithStarSign(data) {
    if (!data) return 0;
    let count = 0;
    for (let key in data) {
        const pkmn = data[key];
        if (pkmn?.caught > 0 && (pkmn?.starsign || pkmn?.starSign)) count++;
    }
    return count;
}

function countPokemonWithCompleteStarSign(data) {
    if (!data) return 0;
    let count = 0;
    for (let key in data) {
        const pkmn = data[key];
        if (pkmn?.caught > 0) {
            const list = pkmn?.starsignList || pkmn?.starSignList || [];
            if (list.length >= 6) count++;
        }
    }
    return count;
}

function countPokemonWithSpecificStarSign(data, signName) {
    if (!data) return 0;
    let count = 0;
    for (let key in data) {
        const pkmn = data[key];
        if (pkmn?.caught > 0) {
            const list = pkmn?.starsignList || pkmn?.starSignList || [];
            if (list.includes(signName)) count++;
        }
    }
    return count;
}
...
function countMegaStones(data) {
    ...
    let count = 0;
    for (let key in data) {
        // Vérifier que c'est bien une méga-gemme (type 'mega' dans items)
        if (items[key]?.type === 'mega' && data[key]?.got > 0) count++;
    }
    return count;
}
...
function getMegaStoneName(megaPkmnName) {
    ...
    // Vérifier si le nom se termine par X ou Y (Charizard X/Y, Mewtwo X/Y)
    const match = baseName.match(/^(.+?)(x|y)$/);
    if (match) {
        // charizard + x -> charizarditeX
        return match[1] + 'ite' + match[2].toUpperCase();
    }
    
    // Cas normal: venusaur -> venusaurite
    return baseName + 'ite';
}

// Helper: vérifie si un Pokémon méga a une méga-gemme associée
function hasMegaStone(megaPkmnName) {
    // Certains méga comme Rayquaza n'ont pas de pierre (méga-évolution spéciale)
    const stoneName = getMegaStoneName(megaPkmnName);
    // Vérifier que la pierre existe dans items et est de type 'mega'
...
function hasMegaStoneInSave(megaPkmnName, data) {
    ...
    const stoneName = getMegaStoneName(megaPkmnName);
    // Vérifier que le joueur a la pierre dans sa save (got > 0)
    // Les pierres méga se terminent par 'ite', 'iteX' ou 'iteY'
    const isMegaStone = items[stoneName]?.type === 'mega' || 
                        stoneName.endsWith('ite') || 
                        stoneName.endsWith('iteX') || 
                        stoneName.endsWith('iteY');
    return isMegaStone && data[stoneName]?.got > 0;
}

function countZCrystals(data) {
    if (!data) return 0;
    const zCrystals = ['buginiumZ', 'darkiniumZ', 'dragoniumZ', 'electriumZ', 'fairiumZ', 'fightiniumZ', 'firiumZ', 'flyiniumZ', 'ghostiumZ', 'grassiumZ', 'groundiumZ', 'iciumZ', 'normaliumZ', 'poisoniumZ', 'psychiumZ', 'rockiumZ', 'steeliumZ', 'wateriumZ'];
    let count = 0;
    for (let zc of zCrystals) {
        if (data[zc]?.got > 0) count++;
    }
    return count;
}

function countZCrystalsAt20(data) {
    if (!data) return 0;
    const zCrystals = ['buginiumZ', 'darkiniumZ', 'dragoniumZ', 'electriumZ', 'fairiumZ', 'fightiniumZ', 'firiumZ', 'flyiniumZ', 'ghostiumZ', 'grassiumZ', 'groundiumZ', 'iciumZ', 'normaliumZ', 'poisoniumZ', 'psychiumZ', 'rockiumZ', 'steeliumZ', 'wateriumZ'];
    let count = 0;
    for (let zc of zCrystals) {
        if (data[zc]?.got >= 20) count++;
    }
    return count;
}
...
// === NOUVELLES FONCTIONS HELPER V2.1.1 ===

function countCompleteEvolutionFamilies(data) {
    if (!data) return 0;
    // Listes de familles d'évolution (pré-évo, évolué, forme finale)
    const families = [
    ...
    for (let family of families) {
        let complete = true;
        for (let pkmn of family) {
            if (!data[pkmn] || data[pkmn].caught <= 0) {
                complete = false;
                break;
            }
        }
...
function countShinyLegendaries(data) {
    if (!data) return 0;
    const legendaries = [
        ...
        'koraidon', 'miraidon', 'walkingWake', 'ironLeaves'
    ];
    let count = 0;
    for (let leg of legendaries) {
        if (data[leg]?.caught > 0 && data[leg]?.shiny) count++;
    }
    return count;
}

function countStarters(data) {
    if (!data) return 0;
    const starters = [
    ...
    for (let s of starters) {
        if (data[s]?.caught > 0) count++;
    }
    return count;
}

function countStarterEvolutions(data) {
    if (!data) return 0;
    ...
    let count = 0;
    for (let s of starterEvolutions) {
        if (data[s]?.caught > 0) count++;
    }
    return count;
}

function countEeveelutions(data) {
    if (!data) return 0;
    const eevees = ['eevee', 'vaporeon', 'jolteon', 'flareon', 'espeon', 'umbreon', 'leafeon', 'glaceon', 'sylveon'];
    let count = 0;
    for (let e of eevees) {
        if (data[e]?.caught > 0) count++;
    }
    return count;
}

function countFossilPokemon(data) {
    if (!data) return 0;
    const fossils = [
    ...
    for (let f of fossils) {
        if (data[f]?.caught > 0) count++;
    }
    return count;
}

function countPseudoLegendaries(data) {
    if (!data) return 0;
    const pseudos = [
        'dragonite', 'tyranitar', 'salamence', 'metagross', 'garchomp',
        'hydreigon', 'goodra', 'kommoo', 'dragapult', 'baxcalibur'
    ];
    let count = 0;
    for (let p of pseudos) {
        if (data[p]?.caught > 0) count++;
    }
    return count;
}

function countBabyPokemon(data) {
    if (!data) return 0;
    const babies = [
        'pichu', 'cleffa', 'igglybuff', 'togepi', 'tyrogue', 'smoochum',
        'elekid', 'magby', 'azurill', 'wynaut', 'budew', 'chingling',
        'bonsly', 'mimeJr', 'happiny', 'munchlax', 'riolu', 'mantyke',
        'toxel'
    ];
    let count = 0;
    for (let b of babies) {
        if (data[b]?.caught > 0) count++;
    }
    return count;
}

function countPremiumShopItems(data) {
    if (!data) return 0;
    // Magazine, Battle Pass, Replicators, and Apricorn/Noigrume items
    const premiumItems = ['magazineSubscription', 'battlePass', 'replicatorUpgradeS', 'replicatorUpgradeE', 'redApricorn', 'blueApricorn', 'yellowApricorn', 'greenApricorn', 'pinkApricorn', 'whiteApricorn', 'blackApricorn'];
    let count = 0;
    for (let item of premiumItems) {
        if (data[item]?.got > 0) count++;
    }
    return count;
}
...
function createChallengesModal() {
    ...
    const texts = {
        ...
        starsign: isFr ? '⭐ Star Signs' : '⭐ Star Signs',
        ...
        ">
            ...
            <div style="padding:15px 20px;border-bottom:1px solid #2a2a4a;display:flex;gap:10px;flex-wrap:wrap;align-items:center">
                ...
                <select id="challenges-filter" onchange="renderChallenges()">
                    ...
                    <option value="starsign">${texts.starsign}</option>
